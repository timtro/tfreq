...
main(int argc, char **argv) {
  ...
  while (getPos() != EOF) {
    ...
    for (i = 0; i < nat; i++) {
      for (j = 0; j < 3; j++) {
        atomVel[i][j] =
            (atomPosC[i][j] - atomPosA[i][j]) * BOHR_RADIUS_SI / (2 * dt);
      }
      printf("%18.9E%18.9E%18.9E\n", atomVel[i][0], atomVel[i][1],
             atomVel[i][2]);
    }
...
inline int getPos() {
  ...
  //
  for (; i < nat; i++) {
    fscanf(posfile, "%lG%lG%lG", &atomPosC[i][0], &atomPosC[i][1],
           &atomPosC[i][2]);
  }
  return a;
}
inline int copyto(double **A, double **B) {
  int i, j;
  for (i = 0; i < nat; i++)
    for (j = 0; j < 3; j++) {
      B[i][j] = A[i][j];
